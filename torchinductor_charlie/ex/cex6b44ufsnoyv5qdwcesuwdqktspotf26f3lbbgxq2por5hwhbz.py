...
@triton_heuristics.pointwise(
    size_hints=[131072], 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*i64', 'in_ptr1': '*fp16', 'in_ptr2': '*fp16', 'out_ptr0': '*fp16', 'out_ptr1': '*fp16', 'xnumel': 'i32'}, 'device': DeviceProperties(type='cuda', index=2, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, multi_processor_count=132, warp_size=32), 'constants': {}, 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]},
...
def triton_poi_fused_index_put_1(in_ptr0, in_ptr1, in_ptr2, out_ptr0, out_ptr1, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp53 = tl.load(in_ptr1 + (1152 + x0 + (1280*x1)), None).to(tl.float32)
    ...
    tmp10 = tmp6 < tmp9
    tmp11 = tl.load(in_ptr1 + (1024 + (2*(x0 // 2)) + (1280*x1)), tmp10, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    tmp21 = tmp12 * tmp20
    tmp22 = tl.load(in_ptr1 + (1025 + (2*(x0 // 2)) + (1280*x1)), tmp10, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    tmp32 = tmp6 < tmp31
    tmp33 = tl.load(in_ptr1 + (1025 + (2*(x0 // 2)) + (1280*x1)), tmp30, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    tmp42 = tmp34 * tmp41
    tmp43 = tl.load(in_ptr1 + (1024 + (2*(x0 // 2)) + (1280*x1)), tmp30, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    tl.store(out_ptr0 + (x0 + (128*tmp4)), tmp52, None)
    tl.store(out_ptr1 + (x0 + (128*tmp4)), tmp53, None)
