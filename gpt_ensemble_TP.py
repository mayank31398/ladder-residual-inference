...
class TransformerBlock(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx: int) -> None:
        ...
        self.feed_forward = FeedForward(config, layer_idx=layer_idx)
        self.ffn_norm = RMSNorm(config.dim, config.norm_eps)
        self.attention_norm = RMSNorm(config.dim, config.norm_eps)

    def forward(self, x: Tensor, input_pos: Tensor, freqs_cis: Tensor, mask: Tensor) -> Tensor:
        h = self.attention(self.attention_norm(x), x, freqs_cis, mask, input_pos)
...
class Attention(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx: int):
        ...
        self.do_all_reduce = config.reduce_pattern[layer_idx]["attention"]
    ...
    def forward(self, x: Tensor, residual: Tensor, freqs_cis: Tensor, mask: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
...
class FeedForward(nn.Module):
    def __init__(self, config: ModelArgs, layer_idx: int) -> None:
        ...
        self.do_all_reduce = layer_idx == config.n_layer - 1 or config.reduce_pattern[layer_idx]["mlp"]
