...
def replace_linear_weight_only_bit8_per_channel(module, target_dtype):
    for name, child in module.named_children():
        if isinstance(child, nn.Linear) and name != "gate":
            setattr(
                module, name, WeightOnlyBit8Linear(child.in_features, child.out_features, target_dtype=target_dtype)
            )
            ...
            setattr(
                module,
                name,
                ConditionalFeedForwardBit8(num_experts, intermediate_size, dim, target_dtype=target_dtype),
            )
        else:
...
class WeightOnlyBit8Linear(torch.nn.Module):
    ...
    def __init__(
        self, in_features: int, out_features: int, bias: bool = True, device=None, dtype=None, target_dtype=None
    ) -> None:
...
class ConditionalFeedForwardBit8(nn.Module):
    ...
    def forward(self, x, expert_indices):
        ...
        expert_outs = torch.einsum("tao, taio -> tai", (x1 * x3), w2_weights) * self.scales2[expert_indices].to(
            x.dtype
        )  # [T, A, D, D]
        return expert_outs
