...
def _apply_tp_moe_ffn(mlp: MOEFeedForward) -> None:
    ...
    mlp.cond_ffn.register_forward_hook(
        lambda _module, _input, output: funcol.all_reduce(output, "sum", list(range(world_size)))
    )
...
def _apply_tp_attn(attn: Attention) -> None:
    ...
    attn.register_forward_hook(
        lambda _module, _input, output: funcol.all_reduce(output[0], "sum", list(range(world_size)))
    )
