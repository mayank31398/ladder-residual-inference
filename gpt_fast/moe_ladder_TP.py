...
from .utils import Attention, KVCache, MoE_Triton, RMSNorm, all_reduce_func, precompute_freqs_cis
...
class LadderMoE(nn.Module):
    def __init__(self, config: ModelArgs) -> None:
        ...
        self.layers = nn.ModuleList(
            LadderMoETransformerBlock(config)
...
class LadderMoETransformerBlock(nn.Module):
    def __init__(self, config: ModelArgs) -> None:
        super().__init__()
        self.attention = Attention(config)
        self.feed_forward = MoE_Triton(config)
